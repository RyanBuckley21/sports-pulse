...
Why some picks are not graded
-----------------------------
There are no betting lines anywhere in this project -- an explicit, documented
deferral (docs/mlb-availability-field-map.md, docs/sports-pulse-schema.md). A
side market (moneyline, first-five moneyline, first-inning runs) resolves from
the box score alone and grades cleanly. A total or a run line does not: "BAL
Under" needs a number, and none exists. implied_total's estimate is not that
number -- its own docstring says it "must never be surfaced as" a market line --
and it is not recoverable after the fact anyway (est_total is not persisted to
the store, the store's `signals` array keeps only one side's OPS/bullpen, and
pitcher_season_era drifts, so a later recomputation would not reproduce the
number the pick was shown with).

So those picks are LISTED with their actual runs and marked UNPRICED, and are
left out of the record rather than graded against an invented line. Passing
`--assume-lines` grades them against explicit conventional reference numbers
(printed in the header whenever it is on); it is opt-in and never silent,
because on 2026-07-22 it moves the day's record from 2-3 to 5-4.
"""
...
# Verdicts that count toward the day's record. Everything else is listed and
# explained, but never folded into the hit rate.
GRADED = ("HIT", "MISS")
...
def grade(pick, game, assume_lines):
    """(result_text, verdict) for one pick against one game record.

    Verdicts: HIT / MISS (count toward the record) · PUSH (no side landed) ·
    UNPRICED (needs a line that does not exist) · PENDING (not resolved yet, a
    ...
    if game is None:
        return "not on this date's schedule", "UNRESOLVED"
    if is_called_off(game):
        ...
        return text, "POSTPONED"
    if not is_final(game):
        return live_state(game), "PENDING"
    ...
    if bt in ("moneyline", "first_five_moneyline", "run_line") and side not in (away, home):
        return "side {!r} is neither team".format(side), "UNRESOLVED"
    if bt == "nrfi_yrfi" and side not in ("NRFI", "YRFI"):
        return "side {!r} is not NRFI/YRFI".format(side), "UNRESOLVED"
    if bt == "team_total" and side.split(" ")[0] not in (away, home):
        return "side {!r} names no team".format(side), "UNRESOLVED"
    if bt in ("game_total", "first_five_total", "team_total") and side.split(" ")[-1] not in ("Over", "Under"):
        return "side {!r} is neither Over nor Under".format(side), "UNRESOLVED"
    ...
    if bt == "moneyline":
        ...
        return (final + " (tie)" if winner is None else final), _side_verdict(side, winner)

    if bt == "first_five_moneyline":
        f_away, f_home, n_inn = innings_runs(game, 5)
        if n_inn < 5:
            return "only {} inning(s) played".format(n_inn), "UNRESOLVED"
        text = "{} {} - {} {} thru 5".format(away, f_away, home, f_home)
        if f_away == f_home:
            return text + " (tie)", "PUSH"
        return text, _side_verdict(side, away if f_away > f_home else home)

    if bt == "nrfi_yrfi":
        f_away, f_home, n_inn = innings_runs(game, 1)
        if n_inn < 1:
            return "no linescore data", "UNRESOLVED"
        runs = f_away + f_home
        text = "{} run{} in the 1st".format(runs, "" if runs == 1 else "s")
        return text, ("HIT" if (side == "YRFI") == (runs > 0) else "MISS")

    if bt == "run_line":
        margin = (a_score - h_score) if side == away else (h_score - a_score)
        text = "{} (margin {:+d})".format(final, margin)
        if not assume_lines:
            return text, "UNPRICED"
        return "{} vs -{}".format(text, ASSUMED_RUN_LINE), ("HIT" if margin >= 2 else "MISS")
    ...
    if bt in ("game_total", "first_five_total", "team_total"):
        ...
        return "{} vs {}".format(text, line), ("HIT" if (actual > line) == over else "MISS")

    return "unknown market", "UNRESOLVED"
...
def render(date, picks, rows, store_size, assume_lines, out=sys.stdout):
    ...
    if assume_lines:
        w("--assume-lines ON: grading totals against ASSUMED reference numbers "
        ...
        w("These are conventional round numbers, NOT real market lines — no line data exists in this project.")
    w()
    w(ROW.format("SCORE", "MATCHUP", "START", "MARKET", "PICK", "RESULT", "VERDICT", ""))
    for pick, game, result, verdict in rows:
        flags = "  ⚑ " + ",".join(pick["flags"]) if pick["flags"] else ""
        w(ROW.format(pick["score"], matchup(pick, game), short_start(pick["start"]),
                     pick["market"], pick["side"], fit(result), verdict, flags))
    w()
    w(summary_line(date, rows))
    pending = sum(1 for r in rows if r[3] in RERUNNABLE)
    if pending:
        w("         {} pick{} unresolved — re-run after they finish.".format(
            pending, "" if pending == 1 else "s"))


def summary_line(date, rows):
    tally = {}
    for _, _, _, verdict in rows:
        tally[verdict] = tally.get(verdict, 0) + 1
    hits, misses = tally.get("HIT", 0), tally.get("MISS", 0)
    n = hits + misses
    rate = "{:.0f}%".format(100.0 * hits / n) if n else "n/a"
    extra = "".join("  ·  {} {}".format(v, k.lower())
                    for k, v in sorted(tally.items()) if k not in GRADED)
    return "Summary  {}:  {}-{} ({}) on {} graded pick{}{}  ·  {} total".format(
        date, hits, misses, rate, n, "" if n == 1 else "s", extra, len(rows))
...
def main(argv=None):
    ...
    for pick in picks:
        ...
        result, verdict = grade(pick, game, args.assume_lines)
        rows.append((pick, game, result, verdict))
